...
def set_th_by_anchors(tkttores, log, name, th_passed):
    dfs = {}
    for k in tkttores.keys():
        if k.split("_")[2] == name:
            dfs[k] = pandas.read_csv(glob.glob(os.path.join(tkttores[k][1], "top*.csv"))[0], index_col=0)['-log10_p']
    df = pandas.concat(dfs).sort_values(ascending=False)
    th = df.iloc[len(dfs) * th_passed]
    log += "set plate threshold at %g" % th
    return th, log
...
def check_and_correct(tkttores, comments, IDs, out_path, cols, lib, max_reads, num_in_ref):
    ...
    if CLEAN_BATCH and (lib == 'AT'):
        min_p, log = set_th_by_anchors(tkttores, log, "Anchor", th_passed=AT_TH_PASSED)
    ...
    for UID in IDs.keys():
        ...
        if True:
            ...
            try:
                ...
                if CLEAN_BATCH and (lib == 'AT'):
                    part_df = df[(df.orig_cnt < 1000) & (df.orig_cnt > MIN_BASE)]
                    res_slope = part_df[part_df['-log10_p'] < (part_df['-log10_p'].quantile(0.95))].fold.median()
                    tmp['fold'] = tmp['fold'] / res_slope
