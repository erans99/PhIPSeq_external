...
if __name__ == '__main__':
    ...
    lfold_df = pandas.read_pickle(os.path.join(cache_path, "log_fold_%s_above%d.pkl" % (lib, MIN_OLIS)))
    lfold_df.index = lfold_df.index.get_level_values(0)

    olis = lfold_df.columns[(lfold_df > 0).sum() > (MIN_APPEAR_IN * len(lfold_df))]
    lfold_df = lfold_df[olis]

    MB_df = pandas.read_pickle(os.path.join(cache_path, "MB_agilent_above200.pkl"))
    ...
    for df, name in [[df_full, ""], [df_part, "_nonCNTRL"]]:
        ...
        bc_src = list(filter(lambda x: '&' not in x, df_info.loc[olis].bac_src_SGB.value_counts().index))
        sub = list(set([x.split("_")[-1] for x in d_SGBs.index]).intersection(bc_src))
        sub.sort()
        cols = list(df_info.loc[df_info[numpy.isin(df_info.bac_src_SGB, sub)].index.intersection(olis)].\
                    sort_values('bac_src_SGB').index)
        part_corrs = corrs.loc[["sSGB__" + x for x in sub]][cols]

        plt.figure(figsize=(15, 2))
        plt.imshow(part_corrs.values, cmap='plasma', aspect='auto')
        plt.xlabel("oligos")
        plt.ylabel("SGBs")
        cbar = plt.colorbar()
        cbar.ax.set_ylabel("spearman correlation", rotation=-90, va="bottom")
        plt.title("Spearman correlation between bacterial abundance and oligo fold change")
        plt.tight_layout()
        plt.savefig(os.path.join(out_path, "bac_src_corrs%s.png" % name))
        plt.close('all')
