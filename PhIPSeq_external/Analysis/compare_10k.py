...
def run_on_inds(k, inds, base, repeat10k, run_log=False, run_spearman=False):
    if ALL:
    ...
    for i, b in enumerate(base):
        ...
        for j, r in enumerate(repeat10k):
            ...
            if run_spearman:
                tmp[r] = comp.corr('spearman').loc[b, r].values[0][0]
            else:
                tmp[r] = comp.corr('pearson').loc[b, r].values[0][0]
...
if __name__ == "__main__":
    ...
    inds['VFDB'] = fold_df.columns.intersection(df_info[df_info.is_toxin].index)
    inds['microbiome'] = fold_df.columns.intersection(df_info[df_info.is_PNP | df_info.is_nonPNP_strains].index)
    inds['PNP'] = fold_df.columns.intersection(df_info[df_info.is_PNP].index)
    inds['EM'] = fold_df.columns.intersection(df_info[df_info.is_EM].index)

    base = meta_df.index[1::2]
    repeat10k = meta_df.index[::2]
    if lib is None:
    ...
    for k in inds.keys():
        for n in ['spearman', "log_fold", "fold"]:
            ...
            plt.xticks(range(0, len(r), 10),
                       [meta_df.loc[x].RegistrationCode + " " + str(meta_df.loc[x].Date.year) for x in base[::10]],
                       rotation='vertical')
            plt.yticks(range(0, len(r), 10),
                       [meta_df.loc[x].old_RegistrationCode + " " + str(meta_df.loc[x].Date.year) for x in repeat10k[::10]],
                       rotation='horizontal')
            cbar = plt.colorbar()
            if n == 'spearman':
