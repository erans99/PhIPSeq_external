...
if __name__ == '__main__':
    ...
    if TYPE == 'exist':
        df = pandas.read_pickle(os.path.join(chache_path, "exist_agilent_above200.pkl"))
        df.fillna(0, inplace=True)
    elif TYPE == 'fold':
        df = pandas.read_pickle(os.path.join(chache_path, "fold_agilent_above200.pkl"))
    ...
    MB_df = pandas.read_pickle(os.path.join(chache_path, "MB_agilent_above200.pkl")).T
    inds = MB_df.index.intersection(df.columns)
    MB_df = MB_df.reindex(inds)
    df = df[inds]

    MB_df[MB_df == MB_df.min().min()] = 0
