...
CHECK = None
EXT = ['age', 'gender']
OLIGO_STAT = [True, False][:1]
INCLUDE_STAT = ['bacterial', 'microbiome', 'PNP', 'VFDB']
TYPE_STAT = ['fold', 'pval', 'exist', 'num_exist']


def get_model(pheno, sbj_inf, cache_path, OLIGO, TYPE, INCLUDE, ext=pandas.DataFrame()):
    if OLIGO:
            ...
            for INCLUDE in INCLUDE_STAT:
                ...
                if len(EXT) == 0:
                    ...
                    for pheno in list(bonf['pheno']) + pheno_2:
                        get_model(pheno, sbj_inf[pheno], cache_path, OLIGO, TYPE, INCLUDE)
                else:
                    ...
                    for pheno in res[res['log2_p_val_ks'] < numpy.log2(0.05)]['pheno']:
                        ...
                        if pheno in ext:
                            ext.pop(ext.index(pheno))
                        get_model(pheno, sbj_inf[pheno], cache_path, OLIGO, TYPE, INCLUDE, sbj_inf[ext])
    print("Done")
