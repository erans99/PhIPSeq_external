...
def check_shap(pheno, sbj_inf, cache_path, TYPE, INCLUDE, ext=pandas.DataFrame()):
    ...
    for fold in range(NUM_FOLDS):
        ...
        if pr:
            if len(set(output.loc[locs].values)) > 2:
                ...
                train_r = scipy.stats.pearsonr(output.loc[train_set].values, train_predict)
                test_r = scipy.stats.pearsonr(output.loc[test_set].values, test_predict)
            else:
        ...
        for INCLUDE in INCLUDE_STAT:
            ...
            if len(EXT) == 0:
                ...
                for pheno in pheno_2:  # bonf['pheno'] + pheno_2:
                    check_shap(pheno, sbj_inf[pheno], cache_path, TYPE, INCLUDE)
            else:
                ...
                for pheno in res[res['log2_p_val_ks'] < 0]['pheno']:
                    ...
                    if pheno in ext:
                        ext.pop(ext.index(pheno))
                    check_shap(pheno, sbj_inf[pheno], cache_path, TYPE, INCLUDE, sbj_inf[ext])
    print("Done")
